...
import db from '../../../../../db';
import { statuses, referencesAttachTemplateToTaskTable } from '../../../../../db/schemas/references/attachTemplateToTask';
		...
		for (const status of statuses) {
			const statusCountResult = await db
				...
				.where(
					and(
						...whereConditions,
						eq(referencesAttachTemplateToTaskTable.status, status),
					),
				);
