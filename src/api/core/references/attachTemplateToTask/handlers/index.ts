...
type IStatuses = Pick<
	InferSelectModel<typeof referencesAttachTemplateToTaskTable>,
	'status'
>;
...
export const indexHandler = async (
	...
	try {
		...
		if (id) {
			const record =
				await db.query.referencesAttachTemplateToTaskTable.findFirst(
					{
						where: eq(
							referencesAttachTemplateToTaskTable.id,
							Number(id),
						),
						with: {
							taskTemplate: {
		...
		if (userId !== SUPER_ADMIN_ID) {
			whereConditions.push(
				eq(
					referencesAttachTemplateToTaskTable.createdBy,
					userId,
				),
			);
		}
		...
		if (status !== 'all') {
			whereConditions.push(
				eq(
					referencesAttachTemplateToTaskTable.status,
					status,
				),
			);
		} else {
		...
		const records =
			await db.query.referencesAttachTemplateToTaskTable.findMany(
				{
					where: whereClause,
					orderBy: asc(
						referencesAttachTemplateToTaskTable.createdAt,
					),
					limit: _dataPerPage,
					offset,
					with: {
