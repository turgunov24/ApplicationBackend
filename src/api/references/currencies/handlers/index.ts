...
/**
 * @swagger
 * /api/references/currencies:
 *   get:
 *     summary: Get currencies list or single currency
 ...
 *         schema:
 *           type: string
 *         description: Currency ID to get single currency
 *       - in: query
 *         name: currentPage
 *         schema:
 *           type: string
...
type IStatuses = Pick<
	InferSelectModel<typeof referencesCurrenciesTable>,
	'status'
>;
...
export const indexHandler = async (
	...
	try {
		...
		if (status !== 'all') {
			...
			whereConditions.push(ne(referencesCurrenciesTable.status, 'deleted'));
		}
		...
		if (search) {
			...
			whereConditions.push(
				or(
					ilike(referencesCurrenciesTable.nameUz, searchTerm),
					ilike(referencesCurrenciesTable.nameRu, searchTerm),
				),
			);
		}
